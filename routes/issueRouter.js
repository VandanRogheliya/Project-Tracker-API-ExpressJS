...
issueRouter
  ...
  .put(cors.corsWithOptions, authenticate.verifyUser, (req, res, next) => {
    ...
    Issues.findById(req.params.issueId).then((issue) => {
      if (issue) {
        let index = req.user.organizations
          .map((e) => e.organization)
          .indexOf(issue.organization);
        if (
          issue.reporter.equals(req.user._id) ||
          (index >= 0 && req.user.organizations[index].admin)
        ) {
          if (
            req.body.title ||
            req.body.details ||
            ...
            Users.findOne({ username })
              .then((user) => {
                ...
                let indexOrg = user.organizations
                  .map((e) => e.organization)
                  .indexOf(req.body.organization);

                if (indexOrg === -1) {
                  var err = new Error(
                    "User not Authorized to become Assignee/Reviewer"
                  );
                  err.status = 403;
                  return next(err);
                }
            ...
            if (req.body.attachment.edit) {
              let index = issue.attachments
                .map((e) => e._id)
                .indexOf(req.body.attachment.attachmentId);

              issue.attachments[index].fileName =
                req.body.attachments[0].fileName;
              issue.attachments[index].fileLink =
                req.body.attachments[0].fileLink;
